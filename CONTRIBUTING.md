...
-   Open a [Pull Request](https://github.com/eclipse-thingweb/ui-wot/pulls)
...
```
git clone https://github.com/eclipse-thingweb/ui-wot
```
...
```
feat(components): add support for magic
...
Eclipse Thingweb uses `prettier` to enforce a consistent coding style.
Please use `npm run format` to automatically apply fixes before committing any changes.
